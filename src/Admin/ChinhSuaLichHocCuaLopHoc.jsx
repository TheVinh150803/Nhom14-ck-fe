...
class ChinhSuaLichHocCuaLopHoc extends Component {
  constructor(props) {
    super(props);
    this.state = {
      scheduleId: "",
  ...
  componentDidMount() {
    ...
    if (location.state && location.state.schedule) {
      const { schedule, classId, classInfo } = location.state;
      const sessionParts = schedule.group.match(/Tiết (\d+) → (\d+)/);
      const session = sessionParts ? `${sessionParts[1]}-${sessionParts[2]}` : "";

      this.setState({
        scheduleId: schedule.id_buoihoc,
        subject: schedule.subject,
        session,
        lecturer: schedule.teacher,
        room: schedule.room,
        date: schedule.date, // Chỉ sử dụng ngày học
        classId,
        classInfo,
      });
    }
  ...
  handleSave = async () => {
    try {
      const token = localStorage.getItem("token");
      const { scheduleId, session, room, date } = this.state;
      ...
      await axios.put(
        `https://webdiemdanh-1.onrender.com/api/admin/capnhatlichhoc/${scheduleId}`,
        {
          tietbd,
          tietkt,
          phonghoc: room,
      ...
      this.setState({ error: error.message || "Không thể cập nhật lịch học" });
    }
  };
...
export default withNavigation(ChinhSuaLichHocCuaLopHoc);